...
finetune=$backwards/reports/experiments/audio_mae/128x320/wear_ssl/2d/imu_i3d/rgb
...
do
    accelerate launch --main_process_port $MASTER_PORT extract_feats_mae.py \
        --config $config \
        --config_mae $config_mae/split_$split/wear_combined_rgb_pt.yaml \
        --finetune $finetune/split_$split/mask_ratio{$mask_ratio}_mae_vit_base_patch16/accelerator_state \
        --seconds 2 \
